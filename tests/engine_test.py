...
def test_e_assign_literal_number() -> None:
    assert run_e("this = 42") == ("42", None, None)


def test_e_assign_literal_string() -> None:
    assert run_e("this = 'OK'") == ("OK", None, None)


def test_e_assign_variable_number() -> None:
    script = """
        value = 7
        this = value
    """
...
def test_e_assign_variable_string() -> None:
    script = """
        status = 'READY'
        this = status
    """
...
def test_e_hash_lookup_with_unit() -> None:
    script = """
...
def test_e_basic_arithmetic() -> None:
    assert run_e("this = #A * 2", "A=5") == ("10", None, None)


def test_e_parentheses_precedence() -> None:
    script = """
        foo = 2
        bar = 3
        this = (foo + #A) * (bar - 1)
...
def test_e_if_else_branching() -> None:
    script = """
        this = 0
...
def test_e_comparison_operator() -> None:
    script = """
        this = 0
        if #A gt #B
         this = 1
...
def test_e_logical_operator() -> None:
    script = """
        this = 0
...
def test_e_comparison_and_logical() -> None:
    script = """
        this = 0
...
def test_e_comparison_logical_with_parentheses() -> None:
    script = """
        this = 0
...
def test_e_for_loop_accumulates_values() -> None:
    script = """
        total = 0
        for I = 1 TO 3
         total = total + I
...
def test_e_deep_nesting_respects_limits() -> None:
    script_ok = """
        total = 0
        for A = 1 TO 2
         for B = 1 TO 2
...
def test_e_roundjisb_preserves_trailing_zero() -> None:
    assert run_e("this = roundjisb(#A, 2, 1)", "A=1.2") == ("1.20", None, None)


def test_e_floor_positive_value() -> None:
    assert run_e("this = floor(#A, 1)", "A=12.39") == ("12.3", None, None)


def test_e_trunc_negative_toward_zero() -> None:
    assert run_e("this = trunc(#A, 1)", "A=-1.29") == ("-1.2", None, None)


def test_e_print_outputs() -> None:
    script = """
        this = 5
        print(this, 'ED')
    """
...
def test_e_print2_outputs() -> None:
    script = """
        this = 5
        print2(this, 'RP')
    """
...
def test_e_max_min_with_multi_measurements() -> None:
...
def test_e_string_functions_is_char_strlen() -> None:
...
def test_e_strcat_statement_updates_target() -> None:
    script = """
        B = '10'
        strcat(B, '.00')
        this = B
...
def test_e_strncpy_statement_uses_shift_jis_bytes() -> None:
    script = """
...
def test_e_string_functions_empty_and_space() -> None:
...
def test_e_multi_unit_and_holder_inputs() -> None:
    script = """
...
def test_r_uses_input_value_on_rhs() -> None:
    assert run_r("this = this + 3", "7") == (None, "10", "10")


def test_r_varref_defaults_to_zero_before_execution() -> None:
    assert run_r("this = this + 1", "") == (None, "エラー", "エラー")


def test_r_rejects_hash_variables() -> None:
    assert run_r("this = #A", "1") == (None, "エラー", "エラー")


def test_r_print_overrides_this_value() -> None:
    script = """
        this = this + 2
        print(this, 'ED')
    """
...
def test_r_print2_overrides_reported_only() -> None:
    script = """
        print(this, 'ED')
        print2(this, 'RP')
    """
...
def test_r_roundjisb_uses_input_as_this() -> None:
    assert run_r("this = roundjisb(this, 2, 1)", "1.205") == (None, "1.21", "1.21")


def test_r_literal_echo_when_no_this_or_print() -> None:
    script = """
        this = this + 0
    """
...
def test_r_allows_hash_in_literals_and_comments() -> None:
    script = """
...
def test_r_print2_condition_false_keeps_literal() -> None:
    script = """
        if this lt 0.23
         print2(this, '不検出')
        end
...
def test_r_rhs_this_always_uses_input_value() -> None:
    script = """
        this = this + 2
        this = this * 2
    """
...
def test_r_nested_for_and_if_structure() -> None:
    script = """
        total = 0
        for I = 1 TO 2
         if I eq 1
...
def test_r_comments_and_literals_ignore_hash_and_this() -> None:
    script = """
...
def test_e_nested_for_if_structure() -> None:
    script = """
        sum = 0
        for I = 1 TO 2
         if I eq 1
...
def test_e_roundjisb_invalid_argument_count() -> None:
    assert run_e("this = roundjisb(#A, 1)", "A=10") == ("エラー", None, None)


def test_e_roundjisb_preserves_fixed_decimals_through_variables() -> None:
    script = """
...
def test_e_roundjisb_preserves_fixed_decimals_direct_assignment() -> None:
    assert run_e("this = roundjisb(12.300, 3, 1)") == ("12.300", None, None)


def test_e_roundjisb_nested_with_floor_keeps_trailing_zero() -> None:
    script = """
...
def test_e_roundjisb_floor_inline_expression_preserves_decimals() -> None:
    script = "this = roundjisb(floor(0.109, 2), 2, 1)"
    assert run_e(script) == ("0.10", None, None)


# def test_e_for_loop_iteration_limits() -> None:
#     # 反復が制限内なら実行できる
#     script_limit = """
#         total = 0
#         for I = 1 TO 1000000
...
def test_e_floor_invalid_argument_type() -> None:
    assert run_e("this = floor('text', 1)", "") == ("エラー", None, None)


def test_e_trunc_invalid_argument_type() -> None:
    assert run_e("this = trunc(#A, 'bad')", "A=5") == ("エラー", None, None)


def test_e_hash_item_missing_raises_error() -> None:
    assert run_e("this = #MISSING", "") == ("エラー", None, None)


def test_e_strncpy_invalid_start_and_length() -> None:
    assert run_e("this = strncpy('', 'ABC', 0)") == ("エラー", None, None)
    assert run_e("this = strncpy('', 'ABC', 2, -1)") == ("エラー", None, None)


def test_e_str_comp_uses_multi_measurement_indices() -> None:
    script = "this = str_comp(#A, #A, 1, 2)"
    assert run_e(script, "A='OK','NG'") == ("1", None, None)


def test_e_str_comp_default_first_measurement() -> None:
    script = "this = str_comp(#A, 'OK')"
    assert run_e(script, "A='OK','NG'") == ("0", None, None)


def test_e_str_comp_rejects_literal_left_operand() -> None:
    assert run_e("this = str_comp('A', 'B')") == ("エラー", None, None)


def test_e_str_comp_requires_variable_index_in_three_arg_form() -> None:
...
def test_e_str_comp_requires_string_literal_in_three_arg_form() -> None:
    script = """
        idx = 1
...
def test_e_requires_this_assignment() -> None:
    assert run_e("foo = 1") == ("エラー", None, None)


def test_e_missing_end_reports_error() -> None:
    script = """
        if 1 eq 1
         this = 1
    """
...
def test_e_for_with_zero_step_reports_error() -> None:
    script = """
        this = 0
        for I = 1 TO 3 STEP 0
         this = this + 1
...
def test_e_for_with_negative_step() -> None:
    script = """
        total = 0
        for I = 3 TO 1 STEP -1
         total = total + I
...
def test_r_print2_preserves_roundjisb_formatting() -> None:
    script = """
